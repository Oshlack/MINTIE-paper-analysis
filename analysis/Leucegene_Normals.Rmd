...
normals_results <- list.files(normals_results_dir, full.names = TRUE) %>% 
                        ...
                        rbindlist(fill = TRUE) %>%
                        filter(logFC > 5)
# normals_results <- normals_results[!(normals_results$variant_type=='RI' & !as.logical(normals_results$is_contig_spliced)),]
...
# controls_comp <- controls_comp[!(controls_comp$variant_type=='RI' & !as.logical(controls_comp$is_contig_spliced)),]
...
MINTIE Supplementary Figure 9 showing range of variant genes called
...
for (ncon_dir in list.dirs(ncon_dirs, recursive = FALSE)) {
    run <- str_split(ncon_dir, "ncon")[[1]] %>%
                tail(1)
    ncon <- str_split(run, "r") %>% unlist() %>% first() %>% as.numeric()
...
results_summary$n_controls <- results_summary$run %>%
                                as.character() %>%
                                str_split(., pattern = "r") %>%
                                lapply(., first) %>% unlist() %>% as.numeric()
rs <- results_summary %>%
